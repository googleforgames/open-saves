...
type OpenSavesClient interface {
	...
	// QueryRecords performs a query and returns matching records.
	QueryRecordsStream(ctx context.Context, in *QueryRecordsRequest, opts ...grpc.CallOption) (OpenSaves_QueryRecordsStreamClient, error)
...
func (c *openSavesClient) QueryRecordsStream(ctx context.Context, in *QueryRecordsRequest, opts ...grpc.CallOption) (OpenSaves_QueryRecordsStreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &OpenSaves_ServiceDesc.Streams[0], "/opensaves.OpenSaves/QueryRecordsStream", opts...)
	if err != nil {
		return nil, err
	}
	x := &openSavesQueryRecordsStreamClient{stream}
...
type OpenSaves_QueryRecordsStreamClient interface {
	Recv() (*RecordResponse, error)
	grpc.ClientStream
}

type openSavesQueryRecordsStreamClient struct {
	grpc.ClientStream
}

func (x *openSavesQueryRecordsStreamClient) Recv() (*RecordResponse, error) {
...
type OpenSavesServer interface {
	...
	// QueryRecords performs a query and returns matching records.
	QueryRecordsStream(*QueryRecordsRequest, OpenSaves_QueryRecordsStreamServer) error
	// UpdateRecord updates an existing record. This returns an error and
	// does not create a new record if the key doesn't exist.
...
func (UnimplementedOpenSavesServer) QueryRecordsStream(*QueryRecordsRequest, OpenSaves_QueryRecordsStreamServer) error {
	return status.Errorf(codes.Unimplemented, "method QueryRecordsStream not implemented")
}
...
func _OpenSaves_QueryRecordsStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(QueryRecordsRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(OpenSavesServer).QueryRecordsStream(m, &openSavesQueryRecordsStreamServer{stream})
}

type OpenSaves_QueryRecordsStreamServer interface {
	Send(*RecordResponse) error
	grpc.ServerStream
}

type openSavesQueryRecordsStreamServer struct {
	grpc.ServerStream
}

func (x *openSavesQueryRecordsStreamServer) Send(m *RecordResponse) error {
...
var OpenSaves_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "QueryRecordsStream",
			Handler:       _OpenSaves_QueryRecordsStream_Handler,
			ServerStreams: true,
		},
		{
